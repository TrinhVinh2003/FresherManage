    ...
    public FresherProjectReponse addFresherToProject(Long id, FresherProjectRequest fresherProjectRequest) {
        ...
        for (var fresher : freshers) {
            emailService.sendProjectUpdateEmail(
                    fresher.getEmail(), fresher.getName(), project.getName(), "added", project.getManager());
        }
        return projectMapping.toProjectFresher(project);
    }
    // xóa fresher khỏi project
    @PreAuthorize("hasRole('ADMIN')")
    public FresherProjectReponse removeFreshersFromProject(
            Long projectId, FresherProjectRequest fresherProjectRequest) {
        var project =
                projectRepository.findById(projectId).orElseThrow(() -> new AppException(ErrorCode.PROJECT_NOT_EXIST));
        ...
        freshersToRemove.forEach(fresher -> {
            emailService.sendProjectUpdateEmail(
                    fresher.getEmail(), fresher.getName(), project.getName(), "removed", project.getManager());
        });
